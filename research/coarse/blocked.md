...
| 2026-08-12 | 小红书主页缺口续扫 `6a3e7af2…` | captured | 首屏已知缺口 ID 已清完（含 `6a008bf4`、`69ec7763`）。更早页仍可按需再降频补扫。 |
| 2026-08-13 | 小红书更早页降频补扫（主页已加载 61 条） | captured | 已见 13 个缺口 ID 全部入库（含过年减脂大决战 `695e4d59`）。更早网格若继续慢滚可能还有未加载笔记。 |
...
| 2026-08-13 | 小红书主页更早网格分页（`68ee` 及 11 月缺口） | other | **外部阻断**：登录态主页首屏约 30 条后停在「三分化背部，二头（上）」`69e32c6e`，底部加载圈不消失；冷却约 15 分钟并以笔记点入的完整 `xsec_token` 重进主页后，分页仍不返回更早笔记。搜索「谭成义 背部训练教学」亦空结果。未继续打分页。待更长冷却或人工慢滚后再扫下列缺口。 |
...
- 主页 URL 需带从笔记点入的完整 `xsec_token`；无 token 时首屏之后分页常失败。
- 若网格停在同一 cursor（本轮为 `69e32c6e`）、`feeds-loading` 一直转：停，不要反复滚动；冷却后再试或交人工。
- 单条闭环：打开 → 截图 → 落 raw → 间隔 → 再下一条。

## 小红书待扫队列（2026-08-13 分页阻断时）

主页显示笔记约 114 条；已入库约 90 个 explore ID。未扫主要在更早网格（需主页慢滚取新 token，禁止无 token 直开）。

- **11 月可能缺口**：`6932b210`（2025-12-05 赛后总结）与 `68fda897`（2025-10-26 腿屈伸）之间，约 `690…`–`692…`。
- **此前见过、尚未入库的更早 ID**（token 已过期，须回主页重取；跳过已入库置顶 `68cf88a3`）：
  - `68ee1306` `68ec72de` `68e9ccc0` `68e71044` `68e497f8` `68e1f242` `68df23a9` `68d75d0c` `68d4d630` `68d25373`
  - `68ccd0d8` `68c92cc1` `68c5675b` `68c3ad5e` `68bfd61f` `68beacf4` `68b44bb9` `68b12068` `68ae8b07` `68ac278c`
  - `68a94039` `68a81bf0` `68a5b944` `68a2dfdf` `68a124cd` `689ef8a6` `689b0570` `6899aa04` `6895c4dd` `6891c450`
  - `688f1410` `688c5a2a` `68899d9f` `6885ff09` `6883408e` `6880b98b` `686a6cc1` `686548b9` `6863efc3`
- `6863…` 约 2025-06/07；已扫的 `67c42e93` 为 2025-03-02 DMS D-3。中间 `685…`→`67d…` 可能仍未加载。
