...
if (!taxonomy.includes('status: draft') && !taxonomy.includes('status: draft')) {
  // keep simple: require status key present
}
...
注意：实现时删掉上面那段无意义的重复 `if (!taxonomy.includes('status: draft') && !taxonomy.includes('status: draft'))` 空块，只保留正则判断。
